				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("IpMatnr") === oObject.IpMatnr;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].IpMatnr;
										} else {
											sCurrentId = aItemsNotInTheList[0].IpMatnr;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("HeaderMaterialSet", {
											IpMatnr : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("IpMatnr"),
							title: oBindingContext.getProperty("IpDokar")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "HeaderMaterialSet (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'HeaderMaterialSet (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("IpDokar");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all HeaderMaterialSet to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "HeaderMaterialSet (" + iExpectedLength + ")"}),
									success : function () {
